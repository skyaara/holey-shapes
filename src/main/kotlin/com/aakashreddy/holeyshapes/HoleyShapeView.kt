 ...
 *
 * The shape spins once when it first appears and whenever its shape or seed changes. Android's
 * system animation scale applies automatically. Set [HoleyShapeConfig.animated] to false for a
 * static mark.
 */
@Composable
fun HoleyShapeView(
    ...
    val path = remember(options.shape, options.holes, options.seed) {
    ...
    LaunchedEffect(options.shape, options.seed, options.animated, options.durationMillis) {
        rotation.snapTo(0f)
        if (options.animated) {
            rotation.animateTo(
                targetValue = 360f,
